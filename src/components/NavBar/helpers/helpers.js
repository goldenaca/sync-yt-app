...
export function checkIfIsUrl({ value, setSearchBar, player, roomId, socket }) {
  if (value.includes("youtube.com/watch?v=")) {
    const urlLinkId = value.split("watch?v=")[1].split("&")[0];
    setTimeout(() => setSearchBar(""), 1000);
    loadLocalVideoHandler({ player, roomId, socket, newVideoId: urlLinkId });
  }

  if (value.includes("youtu.be/")) {
    const urlShareId = value.split("tu.be/")[1];
    setTimeout(() => setSearchBar(""), 1000);
    loadLocalVideoHandler({ player, roomId, socket, newVideoId: urlShareId });
  }
}
